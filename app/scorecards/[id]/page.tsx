...
export default function EditScorecardPage() {
    ...
    fetch(`/api/scorecards/${id}`)
      ...
      .then(data => {
        ...
        setSections(data.sections.map((s: any) => ({
          id: s.id, name: s.name,
          criteria: s.criteria.map((c: any) => ({ id: c.id, question: c.question, isCritical: c.isCritical })),
        })))
        setLoading(false)
      })
  }, [id])
  ...
  const updateSection = (id: string, name: string) => setSections(s => s.map(sec => sec.id === id ? { ...sec, name } : sec))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) router.push('/scorecards')
      ...
      <main className="flex-1 px-8 py-7">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold text-slate-900">Edit Scorecard</h1>
          </div>
        ...
        <div className="max-w-2xl space-y-5">
          ...
          <div className="flex items-center justify-between px-1">
            <p className="text-sm text-slate-500">{sections.length} sections · {sections.reduce((a, s) => a + s.criteria.length, 0)} criteria</p>
            <button onClick={addSection} className="text-sm text-blue-600 font-medium">+ Add Section</button>
          </div>
          ...
          {sections.map((section, si) => (
            <div key={section.id} className="bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm">
              <div className="flex items-center gap-3 px-5 py-3 bg-slate-50 border-b border-slate-100">
                ...
                <input value={section.name} onChange={e => updateSection(section.id, e.target.value)}
                  placeholder="Section name"
                  className="flex-1 bg-transparent text-sm font-medium text-slate-800 focus:outline-none" />
                {sections.length > 1 && <button onClick={() => removeSection(section.id)} className="text-slate-300 hover:text-red-400 text-xl">×</button>}
              </div>
              <div className="divide-y divide-slate-50">
                {section.criteria.map(c => (
                  <div key={c.id} className="flex items-center gap-3 px-5 py-3">
                    <div className="w-2 h-2 rounded-full bg-slate-200 flex-shrink-0" />
                    <input value={c.question} onChange={e => updateCriterion(section.id, c.id, { question: e.target.value })}
