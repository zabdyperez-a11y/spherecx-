...
const ICONS: Record<string, string> = {
  ...
  log: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01',
...
const NAV_SECTIONS = [
  ...
  { title: 'Operations', keys: ['HR', 'Scheduling', 'Training', 'Operations'] },
  { title: 'Admin', keys: ['Audit Log', 'Settings', 'Billing'] },
]
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-56 min-h-screen flex flex-col bg-white border-r border-slate-100 flex-shrink-0">
      {/* Logo */}
      ...
      <nav className="flex-1 px-2 py-3 overflow-y-auto">
        {NAV_SECTIONS.map(section => {
          const sectionItems = visibleItems.filter(item => section.keys.includes(item.label))
          if (sectionItems.length === 0) return null
          return (
            <div key={section.title} className="mb-4">
              <p className="px-3 py-1 text-xs font-semibold text-slate-300 uppercase tracking-wider">{section.title}</p>
              {sectionItems.map(item => {
                ...
                return (
                  <Link key={item.href} href={item.href}
                    className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm mb-0.5 transition-all duration-100 group
                    ...
                    <svg className={`w-4 h-4 flex-shrink-0 ${active ? 'text-blue-600' : 'text-slate-400 group-hover:text-slate-600'}`}
                      fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 1.8}>
                      {iconPath.split(' M').filter(Boolean).map((d: string, i: number) => (
                        <path key={i} strokeLinecap="round" strokeLinejoin="round" d={i === 0 ? d : `M${d}`} />
                      ))}
                    </svg>
                    <span className="truncate">{item.label}</span>
                    {active && <div className="ml-auto w-1 h-4 bg-blue-500 rounded-full flex-shrink-0" />}
                  </Link>
                )
              })}
            </div>
      ...
      <div className="px-3 py-3 border-t border-slate-100">
        {role === 'SUPER_ADMIN' && (
          <div className="mx-1 mb-2 px-2 py-1 bg-indigo-50 rounded-md text-center">
        ...
        <div className="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-slate-50 transition-colors">
          ...
          <button onClick={signOut} title="Sign out" className="text-slate-300 hover:text-slate-500 flex-shrink-0 transition-colors">
