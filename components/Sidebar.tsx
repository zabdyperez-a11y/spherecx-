...
import { ROLE_LABELS, ROLE_COLORS, NAV_ITEMS } from '@/lib/permissions'
...
const ICONS: Record<string, JSX.Element> = {
  ...
  log: <><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 9.776c.112-.017.227-.026.344-.026h15.812c.117 0 .232.009.344.026m-16.5 0a2.25 2.25 0 00-1.883 2.542l.857 6a2.25 2.25 0 002.227 1.932H19.05a2.25 2.25 0 002.227-1.932l.857-6a2.25 2.25 0 00-1.883-2.542m-16.5 0V6A2.25 2.25 0 016 3.75h3.879a1.5 1.5 0 011.06.44l2.122 2.12a1.5 1.5 0 001.06.44H18A2.25 2.25 0 0120.25 9v.776" /></>,
  ...
  leave: <><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" /></>,
  onboarding: <><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" /></>,
  payroll: <><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" /></>,
...
export default function Sidebar() {
  ...
  const visibleItems = NAV_ITEMS.filter(item => item.roles.includes('*') || item.roles.includes(role))
  ...
  const iconOverrides: Record<string, string> = {
    'Leave': 'leave', 'Onboarding': 'onboarding', 'Payroll': 'payroll', 'Integrations': 'integrations',
  }
  ...
  return (
    <>
      ...
      <aside
        className={`flex-shrink-0 flex flex-col transition-transform duration-250
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-3">
          {NAV_SECTIONS.map(section => {
            const items = visibleItems.filter(item => section.keys.includes(item.label))
            ...
            return (
              <div key={section.title} className="mb-5">
                ...
                {items.map(item => {
                  ...
                  const iconKey = iconOverrides[item.label] || iconForItem(item.icon)
                  return (
                    <Link key={item.href} href={item.href}
                      onClick={() => setMobileOpen(false)}
                      className="flex items-center gap-3 px-3 py-2.5 rounded-lg mb-0.5 text-sm font-medium group transition-all duration-100"
                      style={{
        ...
        <div className="px-3 py-3 flex-shrink-0" style={{ borderTop: '1px solid var(--sidebar-border)' }}>
          ...
          <div className="flex items-center gap-2.5 px-2 py-2 rounded-lg transition-colors group"
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold truncate leading-tight" style={{ color: 'rgba(255,255,255,0.85)' }}>{userName}</p>
