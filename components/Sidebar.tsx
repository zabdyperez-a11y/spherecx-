...
export default function Sidebar() {
  ...
  return (
    <aside className="w-56 min-h-screen flex flex-col bg-white border-r border-slate-100" style={{ boxShadow: '1px 0 0 #f1f5f9' }}>
      {/* Logo */}
        ...
        {visibleNav.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              <svg className={`w-4 h-4 flex-shrink-0 transition-colors ${active ? 'text-blue-600' : 'text-slate-400 group-hover:text-slate-600'}`}
                fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 2}>
                {item.icon.split(' ').length > 1
                  ? item.icon.split('M').filter(Boolean).map((d, i) => <path key={i} strokeLinecap="round" strokeLinejoin="round" d={`M${d}`} />)
